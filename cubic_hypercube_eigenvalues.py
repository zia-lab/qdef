...
│                                                        │
│  At then end all eigenvalues are saved to a large .h5  │
│                         file.                          │
│                                                        │
...
h5_filename = '/users/jlizaraz/scratch/out/qdef/tsk_hypercube_eigenvalues_only.h5'
...
if __name__ == '__main__':
    ...
    def cubic_solver(num_electrons, F2, F4, ζ, Dqs, idx_γ, idx_ratio):
        ...
        if any(list(map(np.isnan, [F2, F4, ζ]))):
            ...
            for aDq in Dqs:
                nummatrix = mfun(aDq)
                eigenvals = np.linalg.eigvalsh(nummatrix)
