...
def matrix_simplify(matrix):
    '''
    Faster  simplification  of  a  symbolic  matrix  by  parallelizing the
    simplification across its elements.
    If  the  matrix  is  small  this  might  actually be faster because of
    overheads in parallelization.
    ...
    Returns
    -------
    simpler_matrix (sp.Matrix)
    '''
    num_rows, num_cols = matrix.shape
    row_matrix = matrix.reshape(1,num_rows*num_cols).tolist()[0]
    simpler_matrix = Parallel(n_jobs = num_cores)(delayed(sp.simplify)(mat_element) for mat_element in row_matrix)
    return sp.Matrix(simpler_matrix).reshape(num_cols, num_rows)

def matrix_counter(matrices):
    '''
    Parameters
    ----------
    matrices (list): containing matrices of any shape

    Returns
    -------
    counted_matrices  (list):  containing 2-tuples the first element equal
    to  a  matrix and the second an integer that represents how many times
    that matrix was in the input matrices.
    '''
    matrices_as_tuples = []
    for matrix in matrices:
        tmatrix = tuple(tuple(row) for row in matrix.tolist())
        matrices_as_tuples.append(tmatrix)
    counted_tmatrices = Counter(matrices_as_tuples)
    counted_matrices = [(sp.Matrix(k),count) for k,count in counted_tmatrices.items()]
    return counted_matrices
...
def block_form(matrix):
    ...
    Parameters
    ----------
    matrix (sp.Matrix): a square matrix

    Returns
    -------
    (block_matrix, blocks, col_reordering, row_reordering) (tuple) with

        - block_matrix (sp.Matrix):

        - blocks (list): containing the found blocks

        -  col_reordering  (list):  containing  the  indices  to which the
        columns are reordered in the block form

        -  row_reordering (list): containing the indices to which the rows
        are reordered in the block form
    ...
    block_matrix = sp.Matrix(sp.BlockDiagMatrix(*blocks))
    return block_matrix, blocks, col_reordering, row_reordering
